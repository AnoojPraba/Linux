...
Notes-focused: `31_Generics/01_genericMacro.c` already is the runnable
...
- Pre-C11, "generic" C code (generic containers, generic comparators,
  `qsort`/`bsearch`-style APIs - see `21_QsortBsearch`) worked by type-erasing
...
- In practice they solve different problems: `_Generic` is for "give me one
  ...
  list (`33_LinkedList`) still needs `void *` payloads because its element

...
syscall doesn't provide. `code/30_SystemCalls/01_libcWrapperVsRawSyscall.c`
...
`code/30_SystemCalls/02_errnoAndPerror.c`.
...
*   File system - operating on files/directories and their metadata:
    ...
    `code/11_FileIO/03_syscallIO.c`) and `stat`/`mkdir`/`rmdir`/`creat`/
    `unlink` (file *metadata and directory entries*,
    `code/30_SystemCalls/03_fileSystemCalls.c`).
*   Process control - creating, replacing, and waiting on processes:
    `fork`/`waitpid` (`code/18_Processes/01_forkBasics.c`) and
    `execvp` (`code/18_Processes/02_execFamily.c`).
*   Memory management - controlling a process's virtual address space:
    `mmap`/`munmap` (`code/10_DynamicMemory/`, `code/31_Paging/`) and
    `mprotect`, which changes an already-mapped region's access
    permissions at the page level - see
    `code/30_SystemCalls/05_memoryManagementMprotect.c`, where writing to
    a page after `mprotect`'ing it read-only faults with SIGSEGV.
*   IPC (inter-process communication) - letting separate processes
    exchange data or coordinate: `pipe` (`code/20_IPC/01_pipeCommunication.c`),
    `mmap` with `MAP_SHARED` (`code/20_IPC/02_sharedMemory.c`), and
    process-shared semaphores (`code/20_IPC/03_processSemaphore.c`).
*   Device management - talking to devices through operations that don't
    ...
    codes) - see `code/30_SystemCalls/04_deviceManagementIoctl.c`, which

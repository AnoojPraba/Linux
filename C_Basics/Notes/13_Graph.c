...
The binary search trees in `code/23_BinaryTree/` are the special case;
`code/27_Graph/` covers the general one.
...
*   Adjacency list - an array where index i holds a linked list of i's
    ...
    See `code/27_Graph/01_adjacencyListBFS.c`.
*   Adjacency matrix - an N x N array where matrix[i][j] is 1 (or the edge
    ...
    See `code/27_Graph/02_adjacencyMatrixDFS.c`.
...
*   Shortest path - Dijkstra's algorithm (weighted, non-negative edges) -
    see `code/27_Graph/03_dijkstraShortestPath.c`, which leans on the same

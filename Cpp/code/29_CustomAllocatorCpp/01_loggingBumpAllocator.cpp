...
public:
    ...
    {
        size_t bytesNeeded = objectCount * sizeof(T);
        if (arenaOffset + bytesNeeded <= ARENA_BYTE_COUNT)
        {
            ...
            cout << "  allocate " << bytesNeeded << " bytes from arena (offset now "
                      << arenaOffset << ")\n";
            return result;
        }

        cout << "  allocate " << bytesNeeded << " bytes via malloc (arena exhausted)\n";
        void *fallback = malloc(bytesNeeded);
        if (fallback == nullptr)
        {
            throw bad_alloc();
        }
...
private:
    ...
    static inline size_t arenaOffset = 0;
};
...
int main()
{
    vector<int, LoggingBumpAllocator<int>> values;

    cout << "-- reserve(4) --\n";
    values.reserve(4);

    cout << "-- push_back x5 (forces a reallocation past capacity 4) --\n";
    for (int i = 0; i < 5; i = i + 1)
    {
        values.push_back(i);
    }

    cout << "-- vector goes out of scope --\n";
    return 0;
}

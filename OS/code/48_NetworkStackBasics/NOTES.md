...
`49_SocketProgramming` (TCP client/server), `50_UDPSockets` (UDP client/
server), and multiplexing demos in `51_IOMultiplexing` (select/poll/epoll) -
...
- **`select()`**: caller passes a bitmask (fd_set) of fds to watch; kernel
  ...
  See `51_IOMultiplexing/01_selectMultiplePipes.c`.
- **`poll()`**: like select but uses a resizable array of `struct pollfd`
  ...
  See `51_IOMultiplexing/02_pollMultiplePipes.c`.
- **`epoll()`**: kernel keeps a persistent interest list (`epoll_create()` +
  ...
  (the classic C10K problem). See `51_IOMultiplexing/03_epollServer.c`.
